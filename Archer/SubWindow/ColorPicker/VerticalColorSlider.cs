...
/*****     Filename:          VerticalColorSlider.cs      *****/
...
namespace adobe_color_picker_clone_part_1
{
	...
	public class VerticalColorSlider : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		/// <param scriptName="position">position value of the slider, lowest being at the bottom.  The range
		/// is between 0 and the controls height-9.  The values will be adjusted if too large/small</param>
		/// <param scriptName="Unconditional">If Unconditional is true, the slider is drawn, otherwise some logic 
		...
		private void DrawSlider(int position, bool Unconditional)
		{
			...
			m_iMarker_Start_Y = position;	//	StartCheckAndUpdate the controls marker position


			BufferedGraphicsContext bgf = BufferedGraphicsManager.Current;

			BufferedGraphics bgLeft = bufferGC.Allocate(this.CreateGraphics(), new Rectangle(0, 0, 8, 260));
			...
			arrow[6] = new Point(0,position + 1);		//	G   G
														//	 GGG
			bgLeft.Graphics.Clear(Color.White);
			bgLeft.Graphics.FillPolygon(brush, arrow);	//	Fill left arrow with white
			bgLeft.Graphics.DrawPolygon(pencil, arrow);	//	Draw left arrow border with gray
			bgLeft.Render();

			BufferedGraphics bgRight = bufferGC.Allocate(this.CreateGraphics(), new Rectangle(32, 0, 8, 260));
			...
			bgRight.Graphics.Clear(Color.White);
			bgRight.Graphics.FillPolygon(brush, arrow);	//	Fill right arrow with white
			bgRight.Graphics.DrawPolygon(pencil, arrow);	//	Draw right arrow border with gray
			bgRight.Render();
		}
		...
		private void DrawBorder()
		{
			...
			bufferG.Graphics.DrawRectangle(pencil, 8, 2, 40 - 17, 260 - 5);	//	Draw outer rectangle

			pencil = new Pen(Color.White);
			bufferG.Graphics.DrawRectangle(pencil, 9, 3, 40 - 19, 260 - 7);	//	Draw inner black rectangle
		}
		...
		private void Draw_Style_Hue()
		{
			...
			for ( int i = 0; i < this.Height - 8; i++ )	//	i represents the current line of pixels we want to draw horizontally
			{
				...
				bufferG.Graphics.DrawLine(pen, 10, i + 4, this.Width - 10, i + 4);	//	Draw the line and loop back for next line
			}
		}
		...
		private void Draw_Style_Red()
		{
			for ( int i = 0; i < this.Height - 8; i++ ) //	i represents the current line of pixels we want to draw horizontally
			{
				int red = 255 - (int)Math.Round(255 * (double)i/(this.Height - 8));	//	red is based on the current vertical position
		...
		private void Draw_Style_Green()
		{
			for ( int i = 0; i < this.Height - 8; i++ ) //	i represents the current line of pixels we want to draw horizontally
			{
				int green = 255 - (int)Math.Round(255 * (double)i / (this.Height - 8));	//	green is based on the current vertical position
		...
		private void Draw_Style_Blue()
		{
			for ( int i = 0; i < this.Height - 8; i++ ) //	i represents the current line of pixels we want to draw horizontally
			{
				int blue = 255 - (int)Math.Round(255 * (double)i / (this.Height - 8));	//	green is based on the current vertical position
		...
		private void Reset_Slider(bool Redraw)
		{
			...
			{
				case eDrawStyle.Hue :
					m_iMarker_Start_Y = (this.Height - 8) - (int)Math.Round((this.Height - 8) * m_hsl.H);
					break;
				case eDrawStyle.Saturation :
					m_iMarker_Start_Y = (this.Height - 8) - (int)Math.Round((this.Height - 8) * m_hsl.S);
					break;
				case eDrawStyle.Brightness :
					m_iMarker_Start_Y = (this.Height - 8) - (int)Math.Round((this.Height - 8) * m_hsl.L);
					break;
				case eDrawStyle.Red :
					m_iMarker_Start_Y = (this.Height - 8) - (int)Math.Round((this.Height - 8) * (double)m_rgb.R / 255);
					break;
				case eDrawStyle.Green :
					m_iMarker_Start_Y = (this.Height - 8) - (int)Math.Round((this.Height - 8) * (double)m_rgb.G / 255);
					break;
				case eDrawStyle.Blue :
					m_iMarker_Start_Y = (this.Height - 8) - (int)Math.Round((this.Height - 8) * (double)m_rgb.B / 255);
					break;
			}
		...
		private void ResetHSLRGB()
		{
			switch (m_eDrawStyle)
			{
				...
				case eDrawStyle.Brightness :
					...
					break;
				case eDrawStyle.Red :
					m_rgb = Color.FromArgb(255 - (int)Math.Round(255 * (double)m_iMarker_Start_Y / (this.Height - 9)), m_rgb.G, m_rgb.B);
					m_hsl = AdobeColors.RGB_to_HSL(m_rgb);
					break;
				case eDrawStyle.Green :
					m_rgb = Color.FromArgb(m_rgb.R, 255 - (int)Math.Round(255 * (double)m_iMarker_Start_Y / (this.Height - 9)), m_rgb.B);
					m_hsl = AdobeColors.RGB_to_HSL(m_rgb);
					break;
				case eDrawStyle.Blue :
					m_rgb = Color.FromArgb(m_rgb.R, m_rgb.G, 255 - (int)Math.Round(255 * (double)m_iMarker_Start_Y / (this.Height - 9)));
					m_hsl = AdobeColors.RGB_to_HSL(m_rgb);
					break;
			}
		}
