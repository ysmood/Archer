...
namespace ys
{
	class StrokeParser
	{
		...
		private static void SendStroke(string stroke, StrokeBehavior behavior)
		{
			...
			{
				Archer.Main.Report(Archer.Resource.Exception_StrokeFailed);
			}
		}
		...
		private static bool RunCommonCommand(string stroke)
		{
			...
			IntPtr ptr = ys.Common.GetForegroundWindow();
					...
					new Thread(new ThreadStart(() =>
							{
								...
								if (ys.Common.IsZoomed(ptr))
									ys.Common.ShowWindowAsync(ptr, ys.Common.WindowState.SW_RESTORE);
								else if ((ys.Common.GetWindowLong(ptr, -16) & 0x00020000) == 0x00020000)
									ys.Common.ShowWindowAsync(ptr, ys.Common.WindowState.SW_SHOWMINIMIZED);
							}
						)
					).Start();
					break;
					...
					new Thread(new ThreadStart(() =>
							{
								if (ys.Common.IsZoomed(ptr))
									...
									ys.Common.ShowWindowAsync(ptr, Common.WindowState.SW_SHOWMAXIMIZED);
							}
						)
					).Start();
					break;
					...
					new Thread(new ThreadStart(() =>
							{
								...
								//int GWL_EXSTYLE = -20;
								if ((ys.Common.GetWindowLong(ptr, -20) & 0x00000008) == 0x00000008)
									ys.Common.SetWindowPos(ptr, -2, 0, 0, 0, 0, 0x1 | 0x2);
								else
									ys.Common.SetWindowPos(ptr, -1, 0, 0, 0, 0, 0x1 | 0x2);
							}
						)
					).Start();
					break;
