	...
	public partial class Common
	{
		...
		public static string AutoGetType(string cmd)
		{
			...
			foreach (var m in ms)
			{
				...
				if (m.IsStatic && m.Name == (sr.ReadLine() + "").Trim())
				{
					return "i";
				}
				sr.Close();
		...
		public static string GetFileFullPath(string fileName)
		{
			try
			{
				...
				if (File.Exists(fileNameEx))
					return Path.GetFullPath(fileNameEx);
				...
				try
				{
					...
					{
						var fullPath = Path.Combine(path, fileNameEx);
		...
		public static string RunTempScript(string content, string extention, params string[] args)
		{
			...
			else
			{
				...
				{
					p.StartInfo.Arguments += "\"" + item.Replace("\"", "\"\"") + "\" ";
				}
			}
			p.Start();
		...
		public static bool HaveAuthorityRight()
		{
			try
			{
		...
		public static string HttpPost(string url, string postData, string cookie)
		{
			...
			StreamReader reader = new StreamReader(reStream);
		...
		public static string HttpUpload(string url, Dictionary<string, string> header, string filePath)
		{
			...
			byte[] result = wc.UploadFile(url, "POST", filePath);
